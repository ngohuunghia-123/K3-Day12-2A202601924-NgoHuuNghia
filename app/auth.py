...
def verify_api_key(
    ...
    2. Nếu ``x_api_key`` là None hoặc không khớp → raise 401.
    3. So sánh bằng ``secrets.compare_digest`` — chống timing attack:
       ``==`` dừng tại ký tự đầu khác nhau, rò rỉ thông tin về khóa.
    4. Hợp lệ → trả về ``x_user_id`` nếu client gửi, ngược lại ANONYMOUS_USER.
    """
