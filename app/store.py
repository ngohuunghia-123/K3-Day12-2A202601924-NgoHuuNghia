...
class ConversationStore:
    ...
    def ping(self) -> bool:
        ...
        Nuốt mọi exception và trả False — một exception thoát ra sẽ biến
        readiness probe thành lỗi 500.
        """
        try:
            self.client.ping()
            return True
    ...
    def append(self, user_id: str, role: str, content: str) -> None:
        ...
        ltrim giữ HISTORY_MAX_MESSAGES message gần nhất để prompt không phình vô hạn.
        expire để hội thoại cũ tự hết hạn, khỏi phải dọn tay.
        """
        key = self._key(user_id)
        self.client.rpush(key, json.dumps({"role": role, "content": content}, ensure_ascii=False))
        # ltrim(key, -N, -1) giữ N phần tử CUỐI (mới nhất)
